...
Head: <!DOCTYPE html>, <html lang="en">, UTF-8 meta, viewport meta, <title>Topic</title> where Topic is the chapter or unit name only, in the professor's wording (Supply and Demand; Trade Barriers and Evaluating Markets), with no course code, no term code, and no season or year, then these lines exactly as written (they select the current house stylesheet and script; never use an older version number), followed by the engine placeholder on its own line, and the MathJax block only if the notes contain equations:
...
No label may touch or cover a curve, a point, an arrow, or another label. This applies to every label: curve names (D, S₁), point names (Equilibrium, Point 1), axis values (P₁, $30, 120), brace labels, and preset-generated labels. Place each label in the open space nearest to what it names. For a point where two curves cross, the clear space is the wedge directly to the right (or left) of the point, level with it — not above or below it, where the curves run — and use dx/dy, ldx/ldy, or lstart to move it. Before emitting, check each label against every curve by computing where the curve passes at the label's x-position: if the curve's y there is within the label's height, move the label. If no clear spot exists, shorten the label or drop it and put the words in the caption instead.
