...
(function () {
  ...
  function buildPanel(cfg) {
    ...
    g.appendChild(el('text', {class: 'axlbl', x: O.x + PW + 18, y: O.y + 16}, ax.x || 'Q'));
    g.appendChild(el('text', {class: 'axlbl', x: O.x - 26, y: 22}, ax.y || 'P'));
    ...
    (cfg.curves || []).forEach(function (c) {
      ...
      grp.appendChild(el('text', {class: 'clbl', x: X(lp[0]) + (c.ldx || 6), y: Y(lp[1]) + (c.ldy || (up ? 2 : 6)), fill: cc}, c.label || c.id));
