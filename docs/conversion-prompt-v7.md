<!-- v7 (2026-09-23; dates 2026-09-24). What changed from v6:
  * Class dates were only required to survive PDF input, and the final
    checklist did not mention them at all. Every input's date lines must now
    survive, and check_file.py --source fails on one that did not: concise
    editions of chapters dated over three class meetings were shipping with
    none, because nothing in the procedure or the checker looked.
...
Each class date is <p class="date">Wednesday, 9/2/26</p>, placed immediately before the heading or paragraph that begins that day's material, so it floats into that corner rather than onto a paragraph. A cancelled class is a date line followed by one sentence. Every date line in the input appears in the output, in the same order: a date carries no content of its own, so condensing, merging sections or rebuilding a chapter drops it without leaving a gap anyone notices, and the file still reads as complete. When the output merges two of the input's sections into one, anchor the date to the same point in the content -- the paragraph that day started with -- not to the merged heading, which would date the earlier material to the later class.
...
A guide needs a number at its foot. A dashed line running to a blank spot on the axis -- because its price is suppressed, or will not fit beside its neighbour -- is clutter: drop the guide and keep the dot.
...
PDF input: every paragraph, table row, exam tip, and date in the PDF appears in the HTML, in order, with the same numbers. HTML input: the same, and the date lines especially -- `python scripts/check_file.py <output> --source <the file it was built from>` fails on any that did not survive.
